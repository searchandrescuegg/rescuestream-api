...
- [X] T005 [P] Create `cmd/rescuestream-migrate/main.go` — a separate binary that runs `golang-migrate` against `$DATABASE_URL` and seeds super-admins from `$SUPER_ADMIN_EMAILS`. This is the one-off Fly command invoked pre-cutover. _(Implementation note: extended the existing `cmd/migrate/main.go` instead of creating a parallel binary; same functionality, single binary, matches T004a's justfile recipes.)_
...
- [X] T008 Create migration `internal/database/migrations/000003_v2_structure.up.sql` creating all new tables per `data-model.md` §1 (organizations, teams, users, organization_memberships, super_admins, tags, user_tag_assignments, devices, device_keys, rooms, room_acl_rules, room_devices, sessions). Include indexes noted in data-model. Add nullable `organization_id` + `actor_user_id` to `audit_logs`; add nullable `organization_id`, `room_id`, `device_id` to `streams`.
- [X] T009 Create `000003_v2_structure.down.sql` that drops the new tables in reverse FK order and removes the added columns.
- [X] T010 Create migration `000004_v2_backfill.up.sql`: seed default org + placeholder user (per data-model §5.1 steps 4–6); UPDATE `audit_logs` to set `organization_id`=default and `actor_user_id` from backfilled users; TRUNCATE `streams` (per FR-034); DROP `stream_keys` and `broadcasters` CASCADE.
- [X] T011 Create `000004_v2_backfill.down.sql` — no-op (documented as intentionally non-reversible per spec).
- [X] T012 Create migration `000005_v2_constraints.up.sql`: flip `organization_id` to NOT NULL on `streams`; add partial indexes documented in data-model; add CHECK constraint `organization_memberships.role = 'member' → team_id IS NOT NULL`; add polymorphic target CHECK on `room_acl_rules` via trigger.
- [X] T013 [P] Create `000005_v2_constraints.down.sql` dropping the constraints in reverse order.
- [X] T014 Wire new migrations into `cmd/rescuestream-migrate/main.go` so `SUPER_ADMIN_EMAILS` is passed to the seeding step as a transactional parameter. _(Implementation note: extended the existing `cmd/migrate/main.go` rather than creating a parallel `cmd/rescuestream-migrate/` binary, since `cmd/migrate/` was already wired into the justfile recipes shipped in T004a. Functionally equivalent: `migrate up` now seeds super_admins from `SUPER_ADMIN_EMAILS` post-migration.)_
...
- [X] T026 Implement `internal/domain/session/session.go`: Session struct + invariants (Valid() bool based on revoked_at, expires_at). _(Landed at `internal/domain/session.go` to match the flat-package convention. Includes `SessionRevokeReason*` constants for canonical revocation reasons.)_
- [X] T027 Implement `internal/database/sessionrepo/repo.go`: Create (returns plaintext secret once), FindByKeyID (hot-path, indexed lookup with peppered hash verify), Touch (update last_used_at + sliding expiry), Revoke(id, reason), RevokeAllForUser(userID, reason), EvictExpired. _(Landed at `internal/database/session_repo.go`. Touch is a no-op on revoked rows. Revoke uses COALESCE to preserve the first-recorded reason on duplicate calls. INET round-tripping uses `host()` to strip the netmask suffix.)_
- [X] T028 Implement `internal/service/session/service.go`: LoginComplete (exchanges Google id_token for session keypair), Logout, RevokeAllForUser, ValidateSignedRequest. Functional-options constructor for pepper, expiry window, key entropy. _(Landed at `internal/service/session.go`. Mint replaces the planned LoginComplete and takes a pre-validated user_id; the actual Google id_token verification + user upsert is the **next-chunk** wrapper that wires into the OAuth callback handler. Mint generates random `secret`, stores `pepper.Hash(secret)` as both the client signing key and the server-side verification key — see the doc comment in session.go for why the peppered hash is bidirectional for sessions.)_
- [X] T029 Rewrite `internal/handler/middleware.go::AuthMiddleware` per research §2: parse `X-API-Key` → lookup session → peppered-verify HMAC signature → check not-revoked / not-expired → enforce ±5 min timestamp drift → attach `user_id`, `session_id`, `role`, `org_id`, `team_id` to request context. On session-invalidated → problem `session-invalidated` (401). _(Implementation note: `user_id` + `session_id` are attached now via new `userIDKey` / `sessionIDKey` context keys with public `UserIDFromContext` / `SessionIDFromContext` helpers. `role` / `org_id` / `team_id` attachment is deferred until the membership + super_admin repos land in US1/US2 — at that point `TenancyMiddleware` (T030/T031) will resolve and attach those, layered after `AuthMiddleware`. The legacy `APIKeyFromContext` still returns `X-API-Key` for the audit middleware actor field; T032/T033 will switch the actor source to `user_id`.)_
- [X] T030 Implement tenancy middleware in `internal/handler/middleware.go::TenancyMiddleware`: asserts target resource's `org_id` matches caller's `org_id`; super-admin bypass. Emits `not-in-org` problem (403) on mismatch. Emits `no-org-membership` (403) when caller has no membership. _(Implementation note: folded into the existing `AuthMiddleware` rather than added as a separate middleware. Auth resolves a `domain.CallerIdentity` via `service.IdentityResolver` and rejects no-org users with `/problems/no-org-membership` upstream. The path-vs-orgID `not-in-org` check is **service-layer** per research §6 — every tenant-scoped repo method takes an explicit `orgID` and refuses cross-org reads, with `MapDomainError(ErrNotInOrg)` already wired. Handlers landing in US1+ retrieve the caller via `handler.IdentityFromContext` and reject mismatches.)_
- [X] T031 Implement suspended-org denial in tenancy middleware: loads org.status; rejects all non-super-admin callers if suspended. _(Folded into the same identity-resolution step in `AuthMiddleware`. Super-admins bypass; everyone else gets `/problems/org-suspended` (403) when their resolved org is suspended.)_
...
- [X] T038 [P] Integration test in `tests/integration/tenancy_test.go`: org A admin cannot read org B resources; super-admin can. _(Identity-resolution branch of the merged tenancy/auth middleware exercised in `internal/handler/auth_middleware_identity_test.go` — 6 sub-tests covering super-admin bypass, org-admin attachment, member attachment with team_id, no-org-membership rejection, suspended-org rejection for members, and suspended-org bypass for super-admins. The remaining cross-org read test belongs at the service layer and lands with the org service in US1.)_
...
- [X] T042 [P] [US1] Implement `internal/database/superadminrepo/repo.go`: List, Add(user_id, granted_by, seeded_from_env), Remove(user_id), CountRemaining (used to block last-removal). _(Pulled forward from US1 because the auth pipeline's identity resolver needs `IsSuperAdmin`. Landed at `internal/database/super_admin_repo.go` with the full surface (`IsSuperAdmin`, `Get`, `List`, `Add`, `Remove`, `CountRemaining`); `Add` is idempotent via `ON CONFLICT DO NOTHING`. The last-super-admin transactional guard belongs in the super-admin service when T044's handler lands.)_
...
- [X] T044 [P] [US1] Implement `internal/handler/session/superadmin.go`: `GET /super-admins`, `POST /super-admins`, `DELETE /super-admins/{uid}` with `last-super-admin` guard. _(Landed at `internal/handler/super_admin.go` plus `internal/service/super_admin.go`. The last-super-admin guard runs in a SERIALIZABLE-equivalent transaction: `SELECT user_id FROM super_admins FOR UPDATE` locks every row, the result-set length is the count, and the DELETE only proceeds when count > 1. Added `handler.RequireSuperAdmin` authorization wrapper used by the route subrouter. 9/9 integration tests pass against testcontainers Postgres covering: 403 for non-super-admins (no-org and org-admin), list/add idempotent/empty-email reject, remove-success / remove-last-409 / remove-absent-404 / invalid-uuid.)_

### Organizations service + handlers

- [X] T045 [US1] Implement `internal/service/org/service.go`: Create (accepts name, slug, initial_admin_emails; atomically creates org + org-admin memberships for any existing users + invite placeholders for non-existent emails), List (with typeahead `q`), Get, Update, SuspendUnsuspend, Delete (soft-archive rooms + revoke sessions + hard-delete). _(Landed at `internal/service/organization.go`. Core surface: Create / Get / List (search + status filter + pagination) / Update (rename + suspend/unsuspend in one call) / Suspend / Unsuspend / Delete. Slug is normalized + validated against the same regex used for tag keys (data-model §1.6). **Deferred to a follow-up:** initial_admin_emails orchestration and the soft-archive-rooms-then-revoke-sessions cascade on Delete — both depend on the org-admins service that lands with the AddAdmin/RemoveAdmin commit.)_
- [X] T046 [US1] Implement `internal/service/org/admins.go`: AddAdmin(orgID, email) — upserts user, sets/promotes membership with role=org-admin; RemoveAdmin — downgrades or removes membership; both emit audit events and revoke existing sessions for the affected user. _(Landed at `internal/service/org_admins.go` plus the `POST /orgs/{id}/admins` and `DELETE /orgs/{id}/admins/{user_id}` methods on `OrganizationHandler`. AddAdmin upserts the user by email, calls `MembershipRepository.Replace` (atomically overwrites any existing membership row per FR-002), and revokes the user's sessions with reason `role-changed`. RemoveAdmin only removes the membership when it currently exists with role=org-admin in the target org — member-role rows or admins of a different org both return ErrNotFound to avoid cross-tenant leakage. Sessions revoked with reason `member-removed`. Audit emission is **deferred to T048** (org event audit hooks). 10/10 integration tests pass: happy paths, FR-002 cross-org reassignment with session revocation, only-org-admin role check, super-admin authorization gate, blank-email/invalid-uuid/unknown-org error paths.)_
- [X] T047 [US1] Implement `internal/handler/org/handler.go`: `POST /orgs`, `GET /orgs`, `GET /orgs/{id}`, `PATCH /orgs/{id}`, `DELETE /orgs/{id}`, `POST /orgs/{id}/admins`, `DELETE /orgs/{id}/admins/{uid}` wired into the gorilla/mux router in `cmd/rescuestream-api/main.go`. _(Partial — landed at `internal/handler/organization.go` with the `/orgs` and `/orgs/{id}` CRUD routes wired into a `protected.PathPrefix("/orgs").Subrouter()`. Per-route authorization (super-admin vs org-admin-of-target) is enforced inside each handler method via `authzForOrg`. Status-change requests are super-admin-only; renames are org-admin-OK. **`/orgs/{id}/admins` POST and DELETE land in the next commit** with the org-admins service that drives membership upsert + force-revoke-sessions.)_
...
- [ ] T057 [P] [US2] Implement `internal/database/membershiprepo/repo.go`: Upsert (respecting UNIQUE(user_id)), ListByOrg (paginated, filters), GetByUser, RemoveByUser. _(Partial: `GetByUser` was pulled forward in the Phase 2 identity-resolver commit so the auth pipeline can derive role/org_id/team_id. Landed at `internal/database/membership_repo.go`. `Upsert`, `ListByOrg`, `RemoveByUser` still owed by US2.)_
...
- [X] T111 [P] Remove the v1 `EnvKeyStore` + shared `API_SECRET` code; ensure no code path still reads `API_SECRET`. _(Pulled forward from Phase 9 in the same commit that landed T029. `internal/handler/keystore.go` deleted; `Config.APISecret` field removed; `docker-compose.yml` swapped to `SESSION_SECRET_PEPPER`; the `fly.toml` comment-list trimmed. No source path reads `API_SECRET` any longer.)_
